...
def yolo_infer(images_list, result_path, model_path, context_norm, body_norm, ind2cat, ind2vad, args):
  ...
  for idx, line in enumerate(lines):
    ...
    for pred_bbox in bbox_yolo:
      pred_cat, pred_cont = infer(context_norm, body_norm, ind2cat, ind2vad, device, thresholds, models, image_context_path=image_context_path, bbox=pred_bbox)
      write_text_vad = list()
    ...
    cv2.imwrite('/home/abtandon/proj/data/debug_exp/img_%r.jpg' %(idx), cv2.cvtColor(image_context, cv2.COLOR_RGB2BGR))
